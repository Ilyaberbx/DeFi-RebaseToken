...
contract TokenAndPoolDeployer is Script {
    function run() public returns (RebaseToken token, RebaseTokenPool pool) {
        ...
        vm.startBroadcast();
        token = new RebaseToken(msg.sender);
...
contract VaultDeployer is Script {
    function run(address tokenAddress) public returns (Vault vault) {
        vm.startBroadcast();
        vault = new Vault(IRebaseToken(tokenAddress));
        IRebaseToken(tokenAddress).grantMintAndBurnRole(address(vault));
        vm.stopBroadcast();
    }
}
